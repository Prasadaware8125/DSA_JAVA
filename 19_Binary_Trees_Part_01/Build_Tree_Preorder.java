import java.util.LinkedList;
import java.util.Queue;

public class Build_Tree_Preorder {
    static class Node{
        int data;
        Node left;
        Node right;

        Node(int data) {
            this.data = data;
            this.left = null;
            this.right = null;
        }
    }

    static class BinaryTree {
        static int idx = -1;
        public Node buildTree(int nodes[]) {     // O(n)
            idx++;
            if( nodes[idx] == -1 ) {
                return null;
            } 

            Node newNode = new Node(nodes[idx]);
            newNode.left = buildTree(nodes);
            newNode.right = buildTree(nodes);

            return newNode;
        }

        // Preorder Traversals
        // Steps - Root -> LeftSubtree -> RightSubtree
        public void preorderTraversals(Node root) {      // O(n)
            if( root == null ) {
                // System.out.print("-1 ");
                return;
            }
            System.out.print(root.data+" ");
            preorderTraversals(root.left);
            preorderTraversals(root.right);
        }

        public void inorderTraversals(Node root) {       // O(n)
            if( root == null ) {
                return;
            }
            inorderTraversals(root.left);
            System.out.print(root.data+" ");
            inorderTraversals(root.right);
        }

        public void postorderTraversals(Node root) {     // O(n)
            if( root == null ) {
                return;
            }
            postorderTraversals(root.left);
            postorderTraversals(root.right);
            System.out.print(root.data+" ");
        }

        // Important
        // Microsoft, Facebook, Adobe
        public void levelorderTraversals(Node root) {    // O(2n)
            if( root == null ) {
                return;
            }
            Queue<Node> q = new LinkedList<>();
            q.add(root);
            q.add(null);

            while( !q.isEmpty() ) {
                Node currNode = q.remove();
                if( currNode == null ) {
                    System.out.println();
                    if( q.isEmpty() ) {
                        break;
                    } else {
                        q.add(null);
                    }
                } else {
                    System.out.print(currNode.data+" ");
                    if( currNode.left != null ) {
                        q.add(currNode.left);
                    }
                    if( currNode.right != null ) {
                        q.add(currNode.right);
                    }
                }
            }
        }

        public int height(Node root) {
            if( root == null ) {
                return 0;
            }

            int lh = height(root.left);
            int rh = height(root.right);
            
            return Math.max(lh, rh) + 1;
        }

        public int countNode(Node root) {
            if( root == null ) {
                return 0;
            }

            int ln = countNode(root.left);
            int rn = countNode(root.right);

            return ln + rn + 1;
        }

        public int sumOfNodes(Node root) {
            if( root == null ) {
                return 0;
            }

            int ls = sumOfNodes(root.left);
            int rs = sumOfNodes(root.right);

            return ls + rs + root.data;
        }
    }
    public static void main(String[] args) {
        int nodes[] = {1, 2, 4, -1, -1, 5, -1, -1, 3, -1, 6, -1, -1};
        BinaryTree tree = new BinaryTree();
        Node root = tree.buildTree(nodes);
        // System.out.println(root.data);
        System.out.print("Preorder Traversals : ");
        tree.preorderTraversals(root);
        System.out.println();
        System.out.print("Inorder Traversals : ");
        tree.inorderTraversals(root);
        System.out.println();
        System.out.print("Postorder Traversals : ");
        tree.postorderTraversals(root);
        System.out.println();
        System.out.println("Level Order Traversals : ");
        tree.levelorderTraversals(root);

        System.out.println("\nHeight of tree = "+tree.height(root));

        System.out.println("Number of nodes in tree = "+tree.countNode(root));

        System.out.println("Sum of Nodes = "+tree.sumOfNodes(root));
    }
}